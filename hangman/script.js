 ...
 */
function selectRandomWord() {return WORDS[Math.floor(Math.random()*WORDS.length)];};
...
function keyPressed(e) {
  ...
  let letter = String.fromCharCode(e.keyCode).toLowerCase(); //Find lowercase of the key pressed
  if (word.includes(letter)) { //Checks if the letter guessed is correct
    ...
    if (alphabet.includes(letter) && !wrongLetters.includes(letter)) { //If not already tried
      ...
      if (wrongLetters.length >= 10) {dead()} //If 10 wrong letters have been tried, player loses
    }
    else if (wrongLetters.includes(letter)){ //Tell the user if it has already been tried
      errorMessagesEl.innerText = "You have already guessed this letter"
    }
    else { //Should only run if !alphabet.includes(letter), tells the user to input a valid letter
      errorMessagesEl.innerText = "Please press a valid letter in the English alphabet"
    }
  };
  if (!underscores.includes('_')) {win()}; // If there are no underscores left, the user has won
};
...
function win() {
  ...
  document.removeEventListener("keypress", keyPressed);
  errorMessagesEl.innerText = "You win and saved the hangman! Click to try again";
  document.addEventListener("click", function(){location.reload()}); //Reload is in new function to
    //prevent eventListener from firing upon creation
}

/**
 * List of 1000 common English words.
 */
